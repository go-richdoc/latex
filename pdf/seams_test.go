...
// A document going out through two steps can fail at either, and a caller has
// to be told which. Neither failure can be produced from outside — the parent
// package returns no error today, and the engine typesets an ordinary
// paragraph — so they are reached through the seams.
func TestWhenAStepRefuses(t *testing.T) {
	doc := richdoc.New().P(richdoc.Text{Value: "words"}).Doc()

	t.Run("writing the LaTeX", func(t *testing.T) {
		was := writeLaTeX
		t.Cleanup(func() { writeLaTeX = was })
		writeLaTeX = func(*richdoc.Document) ([]byte, error) { return nil, errors.New("no") }
		_, err := Write(doc, Options{})
		if err == nil {
			t.Fatal("a refusal came back as a PDF")
		}
		if !strings.Contains(err.Error(), "as LaTeX") {
...
func TestStrictAndLenient(t *testing.T) {
	...
	// format wants: a gap in the engine should cost a paragraph rather than
	// the document. A caller writing its own LaTeX asks for strict.
	doc := richdoc.New().P(richdoc.Text{Value: "words"}).Doc()
