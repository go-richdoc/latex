...
func TestWhatSurvivesTheWholeWay(t *testing.T) {
	// The point of this package is that a document keeps its shape across two
	// libraries and a TeX engine. Each of these is a thing a converter can
	// produce, and each is read back out of the finished PDF by poppler rather
	// than trusted.
...
func TestHowManyPagesItCameTo(t *testing.T) {
	...
	// chain, so a caller about to hand somebody an empty file would otherwise
	// have no way to know.
